...
```json
{
  "data": {
    ...
    "driver_iin": "123456789012",
    "driver_phone": "+77001234567",
    "contractor_id": "990e8400-e29b-41d4-a716-446655440004",
    "contractor_name": "ООО Подрядчик",
    "contractor_bin": "123456789012"
  }
}
```
...
- Поля `driver_*` и `contractor_*` заполняются только если транспорт найден в таблице `vehicles` и связан с водителем/подрядчиком
- Если водитель или подрядчик не найдены, соответствующие поля будут отсутствовать в ответе (omitempty)
...
Получение события по ID вместе с фотографиями, информацией о водителе и подрядчике.
...
**Возвращает:**
- `*EventInfo` - информация о событии с фотографиями, данными о водителе и подрядчике
- `error` - ошибка (ErrNotFound если событие не найдено)

**Логика:**
1. Получение события из БД
2. Получение фотографий события
3. Получение данных о водителе из таблицы `drivers` через `vehicles` (по номеру)
4. Получение данных о подрядчике из таблицы `organizations` через `vehicles` (по номеру)
5. Формирование ответа с объединёнными данными
