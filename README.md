...
- `GET /internal/anpr/events` - получение событий по номеру и временному диапазону (требует внутренний токен)
...
- `GET /api/v1/events?plate=123ABC02&from=2025-01-01T00:00:00Z&to=2025-01-31T23:59:59Z&direction=entry&limit=50&offset=0` - поиск событий (требует авторизацию)
...
- `direction` (опционально) - направление движения: `entry` (въезд) или `exit` (выезд)
...
- Время в пути к фото использует часовой пояс Казахстана (GMT+5)

### Internal API (для межсервисного взаимодействия)

- `GET /internal/anpr/events` - получение событий ANPR для расчета объема снега
...
**Параметры запроса (все обязательные, кроме direction):**
- `plate` (обязательно) - номер машины (любой формат, будет нормализован)
- `start_time` (обязательно) - начало временного диапазона в формате RFC3339 (ISO8601)
- `end_time` (обязательно) - конец временного диапазона в формате RFC3339 (ISO8601)
- `direction` (опционально) - направление движения: `entry` (въезд) или `exit` (выезд)
...
**Примечания:**
- Эндпоинт защищен внутренним токеном (`INTERNAL_TOKEN`), который должен быть настроен в переменных окружения
- Возвращает полную информацию о событиях, включая `snow_volume_m3` и `polygon_id`
- Номер машины автоматически нормализуется (убираются пробелы, дефисы, приводится к верхнему регистру)
...
- `INTERNAL_TOKEN` - внутренний токен для межсервисного взаимодействия (защита эндпоинта `/internal/anpr/events`)
