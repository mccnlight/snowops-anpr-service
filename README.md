...
Сравнение периодов для дашборда: день-к-дню, неделя-к-неделе, месяц-к-месяцу.
...
| Параметр | Тип | Обязательно | Описание |
|----------|-----|-------------|----------|
| `mode` | `day` \| `week` \| `month` | Да | Режим сравнения |
| `from` | RFC3339 | Да | Начало текущего периода |
| `to` | RFC3339 | Да | Конец текущего периода |
| `previous_from` | RFC3339 | Нет | Начало предыдущего периода (опционально) |
| `previous_to` | RFC3339 | Нет | Конец предыдущего периода (опционально) |
| `contractor_id` | UUID | Нет | Фильтр по подрядчику (для админов/КГУ) |
| `polygon_id` | UUID | Нет | Фильтр по полигону |
| `vehicle_id` | UUID | Нет | Фильтр по машине |
| `plate` | string | Нет | Поиск по номеру |

**Логика сравнения:**
- Если `previous_from/previous_to` не переданы, предыдущий период считается автоматически:
  - `day`: период сдвигается на 1 день назад
  - `week`: период сдвигается на 7 дней назад
  - `month`: период сдвигается на 1 месяц назад
- Внутри выбранного периода учитываются только события рабочего окна:
  - с `16:00` до `10:00` следующего дня (`Asia/Qyzylorda`)
- Метрики:
  - `total_volume`
  - `trip_count`
- Для `week` и `month` дополнительно возвращаются:
  - `avg_volume_per_day`
  - `avg_trips_per_day`
- Отклонения:
  - `*_percent`
  - `*_color` (`green` / `red` / `gray`)

**Пример запроса:**
```http
GET /api/v1/reports/comparison?mode=week&from=2026-02-10T00:00:00Z&to=2026-02-16T23:59:59Z
Authorization: Bearer <JWT_TOKEN>
```
...
```json
{
  "data": {
    ...
    "current": {
      ...
      "total_volume": 1245.8,
      "trip_count": 312,
      "avg_volume_per_day": 177.97,
      "avg_trips_per_day": 44.57
    },
    "previous": {
      ...
      "total_volume": 1180.4,
      "trip_count": 298,
      "avg_volume_per_day": 168.63,
      "avg_trips_per_day": 42.57
    },
    "deviation": {
      "volume_percent": 5.54,
      "trip_percent": 4.70,
      "avg_volume_percent": 5.54,
      "avg_trips_percent": 4.70,
      "volume_color": "green",
      "trip_color": "green",
