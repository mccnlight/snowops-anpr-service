...
#### `GET /api/v1/reports/weekday-stats`

Статистика для графика **«по дням недели»** на главном дашборде.  
Агрегирует данные по всем неделям выбранного периода.

**Требует авторизацию:** Да (JWT токен)

**Права доступа:**
- Подрядчик (`CONTRACTOR_ADMIN`) — видит только свои данные (автоматически)
- Админы/КГУ — видят все данные, могут фильтровать по подрядчику
...
| `contractor_id` | UUID | Фильтр по подрядчику (для админов/КГУ) |
| `polygon_id` | UUID | Фильтр по полигону |
| `vehicle_id` | UUID | Фильтр по машине |
| `plate` | string | Поиск по номеру |

**Период по умолчанию (если `from` и `to` не переданы):**
- Бэкенд сам рассчитывает **полную текущую неделю (Пн–Вс)** по серверному времени (`Asia/Qyzylorda`).
- Для статистики используется операционный день: **с 16:00 до 10:00 следующего дня**.
- Утренние события (00:00-09:59) относятся к предыдущему операционному дню.
- Это позволяет фронту не рассчитывать даты и просто запрашивать готовые данные для графика.
- Если передаете кастомный период, нужно указывать **обе** границы: и `from`, и `to`.

**Важно для фронта:**
- Ось X уже готова: `MON..SUN` (`iso_weekday: 1..7`, где `1=Пн`, `7=Вс`).
- Для переключения режима оси Y используйте поля:
  - `total_volume` — объём снега (м³)
  - `trip_count` — количество рейсов
- Даже если по какому-то дню нет данных, день все равно вернется в ответе с нулями.
...
**Пример запроса с кастомным периодом:**
```http
GET /api/v1/reports/weekday-stats?from=2026-02-01T00:00:00Z&to=2026-02-12T23:59:59Z
Authorization: Bearer <JWT_TOKEN>
```
...
```json
{
  "data": {
    ...
    "items": [
      { "iso_weekday": 1, "weekday": "MON", "total_volume": 124.5, "trip_count": 11 },
      { "iso_weekday": 2, "weekday": "TUE", "total_volume": 98.0, "trip_count": 9 },
      { "iso_weekday": 3, "weekday": "WED", "total_volume": 0, "trip_count": 0 },
      { "iso_weekday": 4, "weekday": "THU", "total_volume": 110.2, "trip_count": 10 },
      { "iso_weekday": 5, "weekday": "FRI", "total_volume": 135.1, "trip_count": 12 },
      { "iso_weekday": 6, "weekday": "SAT", "total_volume": 76.3, "trip_count": 7 },
      { "iso_weekday": 7, "weekday": "SUN", "total_volume": 65.9, "trip_count": 6 }
    ]
  }
}
```
...
Выгрузка отчетов ANPR в Excel (XLSX) с теми же фильтрами, что и `/api/v1/reports`.

**Требует авторизацию:** Да (JWT токен)

**Права доступа:**
- Подрядчик (`AKIMAT_CONTRACTOR`) — видит только свои события (автоматически)
- Админ/КГУ (`AKIMAT_ADMIN`) — видят все события, могут фильтровать по подрядчику

**Query параметры (все опциональны):**

| Параметр | Тип | Описание | Пример |
|----------|-----|----------|--------|
| `from` | string (RFC3339) | Начало периода | `2026-01-30T00:00:00Z` |
| `to` | string (RFC3339) | Конец периода | `2026-02-02T23:59:59Z` |
| `contractor_id` | string (UUID) | ID подрядчика (ТОО) | `5dcc2e9d-0e1e-47fe-b166-3ea2c0e60147` |
| `polygon_id` | string (UUID) | ID полигона | `uuid-here` |
| `vehicle_id` | string (UUID) | ID транспорта | `uuid-here` |
| `plate` | string | Поиск по номеру (частичное совпадение) | `123ABC` |

**Логика работы:**

1. **Период:**
   - Если `from` и `to` не указаны → последние 24 часа
   - Если указан только `from` → до текущего момента
   - Если указан только `to` → за день до `to`
   - Максимальный диапазон: **30 дней** (иначе 400 Bad Request)

2. **Права доступа:**
   - Подрядчик: видит только свои события (игнорируется `contractor_id` из запроса)
   - Админ/КГУ: видит все события

3. **Ограничения:**
   - Максимум **100,000 строк** (иначе 400 Bad Request)
   - Максимум **30 дней** диапазона (иначе 400 Bad Request)

**Ответ:**

**Успешный ответ (200 OK):**
- `Content-Type`: `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`
- `Content-Disposition`: `attachment; filename="anpr-events_2026-01-30_2026-02-02.xlsx"`
- Body: бинарные данные Excel файла

**Ошибки:**
- `400 Bad Request` — неверные параметры, превышен лимит строк/дней
...
**Колонки:**
1. **ТОО** — название организации-подрядчика
2. **Машина** — марка + модель (например, "KamAZ 6520")
3. **Госномер** — нормализованный или raw номер
4. **Время события** — дата/время (формат: YYYY-MM-DD HH:MM:SS, часовой пояс: Asia/Qyzylorda +05)
5. **Процент** — процент заполнения снегом (например, "95.98%")
6. **Объем** — объем снега в м³ (например, "23.99 м³")

**Группировка:**
- Если `contractor_id` указан → выгрузка одной группы (одна ТОО), без разделителей
- Если `contractor_id` НЕ указан → разбивка по группам:
  - Перед каждой группой вставляется строка-заголовок: **"ТОО: <название>"** (жирный, увеличенный шрифт)
  - Пустая строка между группами
  - Если `contractor_name` отсутствует (NULL) → группируется как **"Не назначено"**
  - После каждой группы выводится итог: **"Итого [Название ТОО]: X рейсов, Y м³"**
  - В конце файла выводится общий итог: **"ВСЕГО: X рейсов, Y м³"**

**Сортировка:**
- По `contractor_name` ASC (NULLS LAST)
- По `plate` ASC
- По `event_time` DESC

**Технические детали:**
- Используется библиотека `github.com/xuri/excelize/v2`
- Потоковая запись (`StreamWriter`) для экономии памяти
- Внутренняя пагинация: чтение из БД порциями по 2000 строк
- Форматирование: закреплена верхняя строка, настроены ширины колонок, формат даты/времени
